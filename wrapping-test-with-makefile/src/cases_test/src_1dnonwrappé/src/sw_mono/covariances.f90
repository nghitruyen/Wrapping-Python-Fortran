    ...
    SUBROUTINE background_error_covariance_matrix( mesh )
      ...
      case ('K(h)')
	    ...
	    do i = 1, n
               x(i) = mesh%crosssection(K_params%cs_start(i))%x ! TO DO : moyenne de start et end pour etre sur la mm grille que
            end do
            ...
            do i = 1, n
               x(i) = mesh%crosssection(K_params%cs_start(i))%x ! TO DO : moyenne de start et end pour etre sur la mm grille que b....
            end do
...
!!!!!!!!!!!!!!!
      ...
      if ( c_hydrograph == 1 ) then
         ...
         if ( c_bathy == 1 ) then
            ...
            allocate( C( n , n ) )
            do i = 1, n
               x(i) = mesh%crosssection(bathy_indices(i))%x
            end do
            ...
            select case( bathymetry )
               ...
               case('parametric')
                  ...
                  allocate( x( n ) )
                  allocate( C( n , n ) )
                  x( : ) = mesh%crosssection( bathy_first : bathy_last : bathy_step )%x
                  call correlation_matrix( n , x , mu_bathy , C )
